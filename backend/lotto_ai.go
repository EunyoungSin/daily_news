...
// 예전 4문장짜리 캐시가 계속 나갔다).
//
// 마지막 규칙("지난주 추천/일치 개수 언급 금지")은 실제로 이 데이터가
// 지금 프롬프트에 전달되고 있어서가 아니다 — buildLottoInsightPrompt는
// frequency/recentAppeared만 넘기고 "지난주 추천 결과"
// (lotto_recommendation_history.go의 matched_count 등)는 전혀 참조하지
// 않는다. 이 규칙은 나중에 누군가 "재미로 지난주 적중 결과도 인사이트에
// 같이 보여주면 좋겠다"며 그 데이터를 무심코 프롬프트에 추가했을 때,
// 모델이 "trend 모드가 이번에 더 잘 맞았다" 같은 비교/분석 문장을
// 만들어내는 것을 막기 위한 선제적 안전장치다.
const insightPromptVersion = "v4"
...
- 지난주 추천 번호가 실제 당첨번호와 몇 개 일치했는지, 또는 어떤 추천 방식(핫넘버 우선/콜드넘버 우선/완전 무작위)이 더 잘 맞았는지는 절대 언급하거나 비교하지 마세요 — 순전히 우연한 일치이며, 특정 방식이 더 우수하다는 인상을 주는 문장은 금지합니다.`
