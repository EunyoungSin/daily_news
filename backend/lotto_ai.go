...
// 예전에는 규칙 2가 "과거 출현 빈도가 다음 회차 당첨 확률에 영향을 주지
// 않는다는 점을 반드시 명확히 언급"이라고 요구해서, 매번 "하지만, 과거
// 출현 빈도가 다음 회차 당첨 확률에 영향을 주지 않는다는 점을 명심해야
// 합니다. 이는 로또의 무작위성에 기인합니다." 같은 긴 문장이 덧붙었다.
// 로또의 무작위성을 알리는 안전장치 자체는 필요하지만, 이미 있는 "※
// 통계적 재미를 위한 분석입니다."라는 짧은 한 줄로 그 역할은 충분하다고
// 판단해 규칙 2를 없애고, 그 disclaimer 한 줄 외에는 더 붙이지 말라고
// 명시적으로 못박았다.
// 규칙 5는 지금 당장은 아무 것도 막지 않는다 — 현재 buildLottoInsightPrompt는
// frequency/recentAppeared만 참고자료로 넘기고, "이번 주 추천 번호"의
// 4단계 필터링 결과(lotto_recommendation_pipeline.go)는 이 프롬프트에
// 전혀 전달하지 않는다. 그래도 미리 넣어두는 이유는, 나중에 누군가
// "AI 인사이트가 이번 주 추천 세트도 언급하면 좋겠다"며 그 결과를 참고
// 자료로 추가할 가능성이 있고, 그때 규칙 3("번호 추천... 절대 금지")만으로는
// "이 필터를 통과한 조합이 더 당첨 가능성이 높다"는 식의 문장까지는
// 못 막기 때문이다 — 그건 특정 번호를 추천하는 게 아니라 필터링
// 과정 자체에 대한 주장이라 별도로 금지해둬야 한다.
...
2. 마지막에 "※ 통계적 재미를 위한 분석입니다." 한 줄만 붙이고, 그 외의 별도 disclaimer 문장은 추가하지 마세요
3. 번호 추천이나 "이 번호가 유리하다"는 표현 절대 금지, 순수 통계 사실만 설명
4. 전체 4~5문장 이내
5. 특정 조합이나 필터링 결과가 참고자료로 함께 주어지더라도, 그 조합이 다른 조합보다 당첨 가능성이 높다는 식의 표현은 절대 사용하지 마세요`
