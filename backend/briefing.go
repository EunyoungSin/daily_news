...
func generateSectionText(ctx context.Context, name, model, systemPrompt, userContent string, allowedNumbers []float64, groundingText, hallucinationFallback string) (simple, detailed string, err error) {
	...
	for attempt := 0; attempt <= maxSectionRegenerations; attempt++ {
		...
		var output briefingLLMOutput
		if jsonErr := json.Unmarshal([]byte(content), &output); jsonErr != nil {
			return "", "", fmt.Errorf("failed to parse %s briefing JSON: %w", name, jsonErr)
		}
		if output.Simple == "" {
			return "", "", fmt.Errorf("%s briefing JSON missing simple field", name)
		}
		if output.Detailed == "" {
			output.Detailed = output.Simple
		}
...
func lookupBriefingSectionCache(ctx context.Context, conn *sql.DB, section string) (briefingSectionCacheRow, bool) {
	...
	err := conn.QueryRowContext(ctx,
		`SELECT data_hash, simple_text, detailed_text, generated_at FROM briefing_section_cache WHERE section = ?`, section,
	).Scan(&row.dataHash, &row.simple, &row.detailed, &row.generatedAt)
	if err != nil {
		return briefingSectionCacheRow{}, false
	}
	return row, true
...
func upsertBriefingSectionCache(ctx context.Context, conn *sql.DB, section, dataHash, simple, detailed string, generatedAt time.Time) error {
	if conn == nil {
		return nil
	}
	_, err := conn.ExecContext(ctx, `
		INSERT INTO briefing_section_cache (section, data_hash, simple_text, detailed_text, generated_at)
		VALUES (?, ?, ?, ?, ?)
		ON DUPLICATE KEY UPDATE data_hash = VALUES(data_hash), simple_text = VALUES(simple_text), detailed_text = VALUES(detailed_text), generated_at = VALUES(generated_at)`,
		section, dataHash, simple, detailed, generatedAt,
	)
	return err
}
...
func getBriefing(ctx context.Context, weather *WeatherData, exchange *ExchangeData, news *NewsData, newsCategory, newsRegion string) (*BriefingData, error) {
	...
	jobs := [3]job{
		{
			...
			userContent:    fmt.Sprintf("[날씨 데이터]: %s\n\n위 데이터를 바탕으로 simple/detailed 두 버전의 한국어 날씨 브리핑 문장을 JSON으로 작성하세요.", weatherJSON),
		...
		},
		{
			...
			userContent:    fmt.Sprintf("[환율 데이터]: %s\n\n위 데이터를 바탕으로 simple/detailed 두 버전의 한국어 환율 브리핑 문장을 JSON으로 작성하세요.", exchangeJSON),
		...
		},
		{
			...
			userContent:           fmt.Sprintf("[뉴스 데이터]: %s\n\n위 데이터를 바탕으로 simple/detailed 두 버전의 한국어 뉴스 브리핑 문장을 JSON으로 작성하세요.", newsJSON),
	...
	for i, out := range outputs {
		...
		if !sectionEmpty {
			...
			detaileds = append(detaileds, out.Detailed)
			if !out.Cached {
				allCached = false
			}
	...
	return &BriefingData{
		Simple:       strings.Join(simples, " "),
		Detailed:     strings.Join(detaileds, " "),
		Cached:       allCached,
