...
// 로또 카드의 "매주 자동 업데이트" 토글이 사용하는 훅 — 실행 상태를
// 가져오고, ON인 동안 5초 간격으로 상태("다음 자동 확인 예정"/"마지막
// 성공")를 폴링하며, start/stop 액션을 제공한다. 실제 데이터(useLotto)와는
// 별개의 관심사다: 이 훅은 "지금 자동 점검이 켜져 있는가"만 알면 되고,
// 회차 데이터 자체는 다루지 않는다.
