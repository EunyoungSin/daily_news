...
로또 섹션은 동행복권 공개 API로 회차 데이터를 MySQL에 영구 저장하고, 그 통계를 Groq로 요약합니다.<br>
...
카테고리(`category`) + 지역(`region`) 조합별로 5분 TTL의 서버 메모리 캐시(`getCachedOrFetchNews`)를
두어, 같은 조합에 대한 `GET /api/news` 요청과 AI 브리핑 내부 조회가 API를 중복 호출하지 않도록
크레딧을 절약합니다. 그래도 카테고리·지역 조합이 많으므로(7개 카테고리 × 2개 지역 = 14가지) 무료
티어에서는 여러 조합을 짧은 시간에 자주 오가면 한도에 도달할 수 있습니다.
...
- 데이터 출처: 동행복권 공개 API(`https://www.dhlottery.co.kr/common.do?method=getLottoNumber&drwNo={회차}`),
  인증 불필요.
- 회차는 2002-12-07(1회차)부터 매주 토요일 추첨되는 주기로 계산한 "이론적 최신 회차"를
  기준으로, DB에 없는 회차만 골라 수집합니다. 최초 실행 시엔 최근 50회를 한 번에
  채우고, 이후에는 매주 신규 1회차만 추가됩니다(`backend/lotto.go`의 `syncLottoDraws`).
- dhlottery API 호출은 세마포어로 동시 요청 수를 6개로 제한합니다.
...
- 뉴스: NewsData.io `/api/1/latest`를 카테고리/지역 조합 하나당 한 번만 호출합니다.
  `getCachedOrFetchNews`의 5분 TTL 메모리 캐시를 `GET /api/news` 핸들러와 AI 브리핑의
  내부 조회가 함께 사용하므로, 같은 조합을 브리핑과 뉴스 카드가 동시에 요청해도 무료 티어
  크레딧을 두 번 쓰지 않습니다. `region=international`일 때만 Groq로 헤드라인 5개를 한
  번의 JSON 모드 호출로 배치 번역하며, 번역 결과는 기사 id 기준으로 별도 캐싱되어
  같은 기사가 남아있는 동안 재번역하지 않습니다(`news_translation_cache`).
