...
- **"최신 회차 = 이 사이클의 결과"라는 잘못된 가정(실제로 겪은 버그,
  2026-08)**: `GET /api/lotto`의 읽기 경로(`getLottoPreviousRecommendationResult`)
  는 예전에 `latestDrwNo`/`latestNumbers`(호출 시점에 DB에 저장된 "최신
  회차")를 그대로 실제 결과로 믿고 대조했습니다. 하지만 자동 수집이 새
  회차를 아직 가져오지 못한 짧은 지연 구간에는 "최신 회차"가 실제로는
  그 이전 사이클의 결과인 경우가 있어(예: 회차 1237이 아직 수집되지
  않아 "최신"이 여전히 1236이던 순간), `previousCycleStart`(현재 시각
  기준으로 계산한 "지난주" 사이클)가 실제로 기다리던 회차(1237)가 아니라
  엉뚱한 이전 회차(1236)의 당첨번호와 대조한 결과가 `matched_count`/
  `matched_numbers`에 그대로 **영구 캐싱**되었습니다(한 번 계산되면
  다시 계산하지 않는 최적화 때문에, 나중에 진짜 회차 1237이 정상적으로
  수집된 뒤에도 스스로 고쳐지지 않았습니다). 실제 사고: 회차 1237
  사이클의 trend 추천 `[1,3,9,20,38,39]`이 회차 1236의 당첨번호
  `[12,18,21,29,34,38]`과 잘못 대조되어 `matched_numbers="38"`로
  저장됨(38은 1236에만 있고 1237의 실제 당첨번호
  `[10,20,23,34,37,40]`에는 없음 — 정답은 "20" 하나였습니다). uniform
  모드는 같은 이유로 정답이 2개(10, 40)인데도 0개로 저장됐습니다.
  지금은 `findLottoDrawClosingCycle`이 "최신 회차가 뭔지"와 무관하게,
  `previousCycleStart + 6일`(그 사이클이 마감되는 정확한 토요일 날짜)로
  `lotto_draws`를 직접 조회해 회차를 찾습니다 — 그 날짜의 회차가 아직
  없으면(수집 지연 중) 다른 회차로 대체하지 않고 조용히 `nil`을
  반환합니다. 이 사고로 이미 잘못 캐싱되어 있던 값은 일회성 스크립트로
  재계산해 직접 수정했습니다(코드베이스에는 남아있지 않습니다 — 근본
  원인이 고쳐졌으니 같은 유형의 오염은 재발하지 않습니다).
