...
`callGroqChat`(`backend/groq.go`)이 이를 위한 전용 재시도를 한 번 더
갖고 있습니다. Groq의 429 에러 메시지에는 "Please try again in
{N}s"처럼 정확히 몇 초 뒤에 재시도하면 되는지가 실려 오는데,
`parseGroqRetryAfterSeconds`가 정규식으로 이 대기 시간을 파싱해서, 그
시간(+0.5초 여유)만큼만 기다린 뒤 **같은 모델·같은 요청**으로 딱 한
번 더 시도합니다. 대기 시간이 10초를 넘거나(초과 폭이 커서 실제로
몇 분을 기다려야 하는 경우) 에러 메시지 형식이 달라 파싱할 수
없으면, 사용자를 무리하게 기다리게 하지 않도록 재시도 없이 바로
실패를 반환해 기존처럼 `stale_fallback`으로 넘어갑니다. 위 문단의
"검증 실패 시 모델 승격" 재시도와는 완전히 별개의 로직입니다 — 이
쪽은 모델을 바꾸지 않고 같은 모델로 아주 짧게 기다렸다 재시도할
뿐이라 서로 간섭하지 않으며, 브리핑 3섹션·뉴스 번역·로또 인사이트가
전부 거쳐 가는 `callGroqChat` 한 곳에만 있어서 호출부를 가리지 않고
모두 적용됩니다.
...
- **환각 방지** (hardFailure, 5종): `findUngroundedNumber`(원본에 없는 숫자),
  ...
  사례(예: "…총으로 쏘려고 시도하여 17년에서 무기징역을" 같은 비문)가 있었습니다.
...
- 뉴스 카드(`NewsCard.tsx`) 자체의 스켈레톤도 최초 로드뿐 아니라 카테고리/지역이
  ...
  재사용해 뚝 끊기지 않게 했습니다.
- 뉴스 카테고리 버튼과 국내/해외 토글(`NewsCard.tsx`)은 `loading`이 `true`인
  동안(즉 카드 자체의 조회가 이미 진행 중인 동안) `disabled` 처리되어
  클릭이 무시되고, `opacity: 0.6` + `cursor: not-allowed`로 흐리게
  표시됩니다 — 카테고리/지역 탭을 빠르게 연달아 전환하면 매번 새 조회가
  겹쳐서 국제 뉴스 번역용 Groq 호출까지 함께 겹치고, 초과 폭이 크면
  TPM(분당 토큰) rate limit에 걸릴 수 있었습니다. `loading`은 응답이
  오든 실패하든 `finally`에서 항상 해제되므로(`useNews.ts`), 응답이
  도착하는 즉시 버튼은 다시 클릭 가능한 상태로 돌아옵니다.
