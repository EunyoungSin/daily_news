...
`maxTokens`는 700에서 500으로 낮췄습니다. `estimateTokenCount`(`groq.go`)로
다소 긴 편에 속하는 실제 뉴스 제목 스타일 번역문 5개(각 35~40자) +
NewsData.io 스타일 article_id(32자)로 구성한 배치 출력 JSON을
측정해보면 약 330토큰이었는데, 700은 이 실제 필요량 대비 여유가
지나치게 커서 다른 Groq 호출들과 TPM 예산을 두고 경쟁하는 상황에서
불필요하게 큰 몫을 예약해두는 셈이었습니다. 다만 `max_tokens`는
생성 가능한 상한일 뿐 실제 소비량이 아니므로 — 정상적으로 성공하는
번역이라면 700이든 500이든 실제 사용 토큰은 거의 동일합니다 — 이
조정의 효과는 평소 TPM 소비를 줄이는 것이 아니라, 모델이 혹시라도
반복 생성 루프에 빠지거나 예상보다 훨씬 긴 응답을 내놓는 예외
상황의 최악의 토큰 소비 상한을 낮추는 데 있습니다.
