...
헤드라인으로라도 새 브리핑을 만들어내기 위한 장치입니다.
...
- **환각 방지** (hardFailure, 5종): `findUngroundedNumber`(원본에 없는 숫자),
  ...
  통화 단위("£25bn", "$6.6bn", "25 million")도 근본 원인은 검증이 아니라 사전 변환 쪽이었습니다
  — `annotateNumericUnits`(`news_number_annotate.go`)의 `numericUnitPattern`이 원래 대문자
  K/M/B와 `$` 기호만 인식해서, `£25bn`처럼 통화 기호가 £/€이거나 단위가 소문자·철자 표기("bn",
  "million")인 경우는 원문 그대로 모델에 전달됐고 모델이 직접 계산하다 "2500억"(10배 과다)으로
  틀렸습니다. 이제 `$/£/€`와 `bn/mn/thousand/million/billion`까지 인식해 LLM에 도달하기 전에
  Go가 정확히 변환합니다(단일 글자 K/M/B는 여전히 대소문자를 구분합니다 — 소문자까지 허용하면
  "100m"(100미터) 같은 표현과 충돌합니다). `findUngroundedNumber`에도 `extractEnglishUnitNumbers`
  라는 같은 종류의 이중 방어선을 추가했습니다 — description 절단 등으로 사전 변환이 어떤 이유로든
  누락돼 원문 단위 표기가 groundingText에 그대로 남아있어도, 정확히 환산된 값은 근거 있는 것으로
  인정합니다. 물론 "2500억"처럼 잘못 계산된 값이나 원문과 무관한 금액은 여전히 걸러집니다.
